...
class TexasBlackTieAndBootsCsvImport extends JobHandler
{
     ...
     * @return array
     */
    protected function read_lines_into_array($file_path, $start_line, $num_lines): array
    {
     ...
     * @throws BatchRequestException
     */
    protected function rememberCSVHeaders($file_path): array
    {
        $entire_file = EE_CSV::instance()->import_csv_to_multi_dimensional_array($file_path);
        if (! is_array($entire_file)) {
            throw new BatchRequestException(
                sprintf(esc_html__('Could not read headers from file $1$s', 'event_espresso'), $file_path)
            );
        }
    ...
    {
        $csv_rows       = $this->read_lines_into_array(
            $job_parameters->request_datum('file_path'),
     ...
     * (in the dashboard ee messages settings) and add add_filter('FHEE__EED_Messages__run_cron__user_wp_cron',
     * '__return_false'); which is currently in-place in EED_Custom_Csv_Importer::set_hooks_both())
     *
     * @param $cols_n_values
     * @throws EE_Error
     * @throws ReflectionException
    ...
    protected function processRow($cols_n_values)
    {
        ...
        if (
            ...
            $data = [$transaction, null, EEM_Registration::status_id_approved];
            try {
                $message_processor    = EE_Registry::instance()->load_lib('Messages_Processor');
                $messages_to_generate =
                    $message_processor->setup_mtgs_for_all_active_messengers('ticket_notice', $data);
                ...
                error_log(
                    'Error sending message for registration ' . $reg->ID() . '. The error was ' . $e->getMessage(
                    ) . ', with stack trace: ' . $e->getTraceAsString()
                );
            }
        }
    }
